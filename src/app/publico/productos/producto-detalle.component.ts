...
@Component({
            ...
            <div class="detalle-prod__acciones">
              @if (redes.whatsapp()) {
                <a class="btn btn--primario"
                   [class.btn--deshabilitado]="p.stock === 0"
                   [href]="p.stock > 0 ? whatsappProducto(p) : null"
                   target="_blank" rel="noopener">
                  Consultar por WhatsApp
                </a>
              }
              <a routerLink="/productos" class="btn btn--linea">Ver más productos</a>
            </div>
  ...
  styles: [`
    ...
    .btn--deshabilitado { opacity: .55; pointer-events: none; }
...
export class ProductoDetalleComponent {
  ...
  whatsappProducto(p: { nombre: string; precio: number }): string {
    const texto = `Hola, quiero consultar por el producto ${p.nombre} (${soles(p.precio)}).`;
