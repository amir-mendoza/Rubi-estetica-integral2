...
@Component({
  ...
  template: `
    <div class="contenedor-petalos" aria-hidden="true">
      ...
      <!-- Definición global del degradado para los pétalos en este componente -->
  ...
  styles: [`
    .contenedor-petalos {
      position: fixed;
      inset: 0;
      z-index: 0; /* Por detrás del contenido, por encima del fondo base */
      pointer-events: none;
      overflow: hidden;
    }
    .petalo-caida {
...
export class PetalosComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Generar 16 pétalos con propiedades distribuidas aleatoriamente
    for (let i = 0; i < 16; i++) {
      this.petalos.push({
        left: `${Math.random() * 100}%`,
        // Usamos delay negativo para que los pétalos ya se encuentren cayendo al abrir la web
